  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="exampleModalLabel">Realizar reporte</h1>
      ...
      <div class="modal-body">
        ...
        <div class="mb-3">
          <label for="n_recin" class="col-form-label">Seleccionar recinto:</label>
          <select class="form-control" id="animal_recinto" name="animal_recinto">
            <option value=""></option>
            <?php
              // Consultar la lista de alimentos desde la base de datos
              $query = "SELECT id_recintos, tipo FROM recintos";
              $result = $conn->query($query);

              // Generar las opciones del menú desplegable
              while ($row = $result->fetch_assoc()) {
                $recintoId = $row['id_recintos'];
                $recintoNombre = $row['tipo'];
                echo "<option value='$recintoId'>$recintoNombre</option>";
              }
            ?>
          </select>
        </div>
      ...
      <div class="modal-footer">
        <button type="button" class="boton" id="reporte_boton">Administrador</button>
        <button type="button" class="boton" id="reporte_boton_medico">Medico</button>
      </div>
    </div>
  </div>
</div>
